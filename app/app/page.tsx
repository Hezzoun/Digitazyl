  ...
  return <main className="app-shell">
    <header className="app-header">
      ...
      <nav className="app-nav" aria-label="Hlavní navigace">
        {[["Domů", LayoutGrid], ["Témata", Compass], ["Komunity", Users], ["Příběhy", MessageCircle], ["Lidé", UserRound]].map(([label, Icon]) => <Link key={label as string} href={label === "Domů" ? "/app" : `/${(label as string).toLowerCase()}`} className={label === "Domů" ? "active" : ""}><Icon /><span>{label as string}</span></Link>)}
      </nav>
      <div className="app-actions"><button aria-label="Notifikace"><Bell /></button><button className="create-top" aria-label="Vytvořit příspěvek"><Plus /></button><Link href="/profil" className="profile-avatar">J</Link></div>
    </header>
    <div className={`app-layout ${mobileNav ? "nav-open" : ""}`}>
      <aside className="app-sidebar glass-panel">
        ...
        <nav className="side-nav">{[["Můj feed", LayoutGrid], ["Objevuj", Compass], ["Uložené", Bookmark], ["Moje příspěvky", PenLine], ["Moje komunity", Users], ["Notifikace", Bell], ["Zprávy", MessageCircle]].map(([label, Icon], i) => <Link key={label as string} className={i === 0 ? "selected" : ""} href="#"><Icon />{label as string}</Link>)}</nav>
